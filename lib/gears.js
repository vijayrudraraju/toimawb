...
// animation data
var _animationTimer = -1;
var _animationData = {x1:0,y1:0,width1:0,x2:0,y2:0,width2:0};
...
function updateNodeGlyphMap(_nodesOnly) 
{
    ...
    for (var i=0;i<count;i++) 
    {
        ...
        {
            nodeGlyphMap.sources.add(sourceSet[i], {curAngle:0,layoutAngle:layoutAngle,layoutX:layoutX,layoutY:layoutY,symbolWidth:symbolWidth,mouseOver:false,subNodes:subNodes,selected:false,visible:true});
        } 
        else 
        {
    ...
    for (var i=0;i<count;i++) 
    {
        ...
        if (!_nodesOnly || subNodes.length() == 0) 
        {
            nodeGlyphMap.sinks.add(sinkSet[i], {curAngle:0,layoutAngle:layoutAngle,layoutX:layoutX,layoutY:layoutY,symbolWidth:symbolWidth,mouseOver:false,subNodes:subNodes,selected:false,visible:true});
        } 
        else 
        {
...
function triggerSourceEditAnimation()
{
    _animationTimer = 0;
}
...
function drawChangingSourceNode()
{
    ...
    console.log(getNodeByName(changingSource,'1'));
...
function drawNodes() 
{
    ...
    for (var i=0;i<nodeGlyphMap.sources.length();i++) 
    {
        ...
        thisAngle = nodeGlyphMap.sources.get(keys[i]).curAngle;
        thisRate = getNodeByName(getCurrentSourcePathPrefix()+keys[i],'1').rate; 
        ...
        // large source nodes
        globalP.resetMatrix();
        globalP.translate(thisX,thisY);
        //globalP.ellipse(thisX,thisY,thisWidth,thisWidth);
        globalP.ellipse(0,0,thisWidth,thisWidth);
        ...
        if (nodeGlyphMap.sources.get(keys[i]).mouseOver) 
        {
            ...
            globalP.text('expand',thisX,thisY+5);
        }
        ...
        if (nodeGlyphMap.sources.get(keys[i]).visible) 
        {
            for (var j=0;j<subNodes.length;j++) 
            {
                ...
                diffX = nodeGlyphMap.sources.get(keys[i]).subNodes.get(subNodes[j]).layoutX - thisX;
                diffY = nodeGlyphMap.sources.get(keys[i]).subNodes.get(subNodes[j]).layoutY - thisY;

                // small source nodes
                globalP.resetMatrix();
                globalP.translate(thisX,thisY);
                globalP.rotate(thisAngle);
                globalP.ellipse(diffX,diffY,
        ...
        nodeGlyphMap.sources.get(keys[i]).curAngle += globalP.PI/32.0 * thisRate;
        if (nodeGlyphMap.sources.get(keys[i]).curAngle > globalP.PI)
        {
            nodeGlyphMap.sources.get(keys[i]).curAngle -= globalP.PI*2;
        }
    }
    ...
    for (var i=0;i<nodeGlyphMap.sinks.length();i++) 
    {
        ...
        thisAngle = nodeGlyphMap.sinks.get(keys[i]).curAngle;

        if (getConnectionsBySinkName(getCurrentSinkPathPrefix()+keys[i]).length !== 0)
        {
            //console.log(getConnectionsBySinkName(getCurrentSinkPathPrefix()+keys[i]));
            var connectedSource = getConnectionsBySinkName(getCurrentSinkPathPrefix()+keys[i])[0].source; 
            //console.log(getNodeByName(connectedSource,'1'), connectedSource);
            thisRate = getNodeByName(connectedSource,'1').rate;
            console.log(thisRate, connectedSource);
        }
        else
        {
            thisRate = 0;
        }
        //thisRate = getNodeByName(getCurrentSinkPathPrefix()+keys[i],'0').rate; 
        ...
        if (nodeGlyphMap.sinks.get(keys[i]).visible) 
        {
            for (var j=0;j<subNodes.length;j++) 
            {
                ...
                diffX = nodeGlyphMap.sinks.get(keys[i]).subNodes.get(subNodes[j]).layoutX - thisX;
                diffY = nodeGlyphMap.sinks.get(keys[i]).subNodes.get(subNodes[j]).layoutY - thisY;
        ...
        nodeGlyphMap.sinks.get(keys[i]).curAngle += globalP.PI/32.0 * thisRate;
        if (nodeGlyphMap.sinks.get(keys[i]).curAngle > globalP.PI)
        {
            nodeGlyphMap.sinks.get(keys[i]).curAngle -= globalP.PI*2;
        }
    }
