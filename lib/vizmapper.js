        ...
        window.AppView = Backbone.View.extend({
            ...
            onReset: function(coll,resp) {
                ...
                if (coll.length === 0 && coll.localStorage.records.length === 0) {
                    ...
                    console.log('local and remote mawbs found');
                }

                signals.removeAll(); 
                var nodes = Blawks.at(0).get('nodes');
                for (var i=0;i<nodes.length;i++) {
                    signals.add(nodes[i].name,nodes[i]);
                }
                updateGraph = true;
                globalP.redraw();
            },
            saveToServer: function() {
                ...
                if ($('#node-name-input').val() !== '') {
                    nodes.push({name:$('#node-name-input').val(),direction:$('#node-type-menu').val()});
                }
                ...
                Blawks.at(0).save({},{success: function() { 
                    ...
                    Blawks.fetch({success: function() { console.log('post-save fetch success');}});
                }});
            },
            initialize: function() {
