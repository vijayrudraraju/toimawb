...
function drawAddButtons() 
{
    ...
    globalP.text('create oscillation source',leftX-248,topY-30);
    globalP.text('@   '+str,leftX-280,topY);
    str = sinkLabelTrace.slice(1,sinkLabelTrace.length).join('');
    globalP.text('create display sink',rightX+50,topY-30);
    globalP.text(str,rightX+50,topY);
...
function drawTraversalGlyphs() {
    ...
    for (var i=0;i<sourceLabelTrace.length;i++) {
        traversalGlyphMap[0].push([[centerX1+80-200,centerY1-120+(i*32),200,28], sourceLabelTrace[i]]);
    }

    for (var i=0;i<sinkLabelTrace.length;i++) {
        traversalGlyphMap[1].push([[centerX2-80,centerY2-120+(i*32),200,28], sinkLabelTrace[i]]);
    }
    ...
    for (var i=0;i<traversalGlyphMap[0].length;i++) {
        ...
        globalP.fill(0);
        globalP.text(sourceLabelTrace[i],centerX1+90-200,centerY1-100+(i*32));
    }
    for (var i=0;i<traversalGlyphMap[1].length;i++) {
        thisX = traversalGlyphMap[1][i][0][0];
        thisY = traversalGlyphMap[1][i][0][1];
        ...
        globalP.fill(0);
        globalP.text(sinkLabelTrace[i],centerX2-70,centerY2-100+(i*32));
    }
}
...
function updateEdgeGlyphMap(signalsOnly) {
    ...
    for (var i=0;i<twiceFilteredConnectionKeys.length;i++) {
        for (var j=0;j<sourceSet.length;j++) {
            ...
            if (twiceFilteredConnectionKeys[i].match(exp1)) {
                ...
                edgeGlyphMap.add(twiceFilteredConnectionKeys[i], {source:sourceLevel[j],sourceChild:sourceSet[j],isSourceSubnode:false});
                ...
                for (var k=0;k<sourceChildren[j].length;k++) {
                    ...
                    if (twiceFilteredConnectionKeys[i].match(exp3)) {
                        edgeGlyphMap.add(twiceFilteredConnectionKeys[i], {source:sourceLevel[j],sourceChild:sourceChildren[j][k].name,isSourceSubnode:true});
                    }
                }
            }
        }
    ...
    for (var i=0;i<connectionSourceMatches.length;i++) {
        for (var j=0;j<sinkSet.length;j++) {
            ...
            if (connectionSourceMatches[i].match(exp1) && !signalsOnly) {
                ...
                for (var k=0;k<sinkChildren[j].length;k++) {
                    ...
                    if (connectionSourceMatches[i].match(exp3)) {
                        edgeGlyphMap.get(connectionSourceMatches[i]).sink = sinkLevel[j];
                        edgeGlyphMap.get(connectionSourceMatches[i]).sinkChild = sinkChildren[j][k].name;
                        edgeGlyphMap.get(connectionSourceMatches[i]).isSinkSubnode = true;
                    }
                }
            } else if (connectionSourceMatches[i].match(exp2)) {
                connectionSinkMatches.push(connectionSourceMatches[i]); 
                edgeGlyphMap.get(connectionSourceMatches[i]).sink = sinkLevel[j];
                edgeGlyphMap.get(connectionSourceMatches[i]).sinkChild = sinkSet[j];
                edgeGlyphMap.get(connectionSourceMatches[i]).isSinkSubnode = false;
            }
        }
    }
    ...
    var cx1, cy1, cx2, cy2;
    for (var i=0;i<keys.length;i++) {
        if (edgeGlyphMap.get(keys[i]).sink != undefined) {

            if (edgeGlyphMap.get(keys[i]).isSourceSubnode) {
                x1 = nodeGlyphMap.sources.get(edgeGlyphMap.get(keys[i]).source).subNodes.get(edgeGlyphMap.get(keys[i]).sourceChild).layoutX;
                y1 = nodeGlyphMap.sources.get(edgeGlyphMap.get(keys[i]).source).subNodes.get(edgeGlyphMap.get(keys[i]).sourceChild).layoutY;
            } else {
                x1 = nodeGlyphMap.sources.get(edgeGlyphMap.get(keys[i]).source).layoutX;
                y1 = nodeGlyphMap.sources.get(edgeGlyphMap.get(keys[i]).source).layoutY;
            }
            if (edgeGlyphMap.get(keys[i]).isSinkSubnode) {
                x2 = nodeGlyphMap.sinks.get(edgeGlyphMap.get(keys[i]).sink).subNodes.get(edgeGlyphMap.get(keys[i]).sinkChild).layoutX;
                y2 = nodeGlyphMap.sinks.get(edgeGlyphMap.get(keys[i]).sink).subNodes.get(edgeGlyphMap.get(keys[i]).sinkChild).layoutY;
            } else {
                x2 = nodeGlyphMap.sinks.get(edgeGlyphMap.get(keys[i]).sink).layoutX;
                y2 = nodeGlyphMap.sinks.get(edgeGlyphMap.get(keys[i]).sink).layoutY;
            }
