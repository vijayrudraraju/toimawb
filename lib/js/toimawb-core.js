...
TOI.MAIN.start = function() {
    ...
    var setActiveType = function(str) {
        if (str !== 'marker') {
            TOI.STATE.activeType = str;
        }
    };
    var setTypeWrite = function(str) {
        var el = document.getElementsByClassName('marker')[0];
        el.className = 'frame-obj marker active';

        console.log('set',str,'on',TOI.STATE.sequence,'type',TOI.STATE.activeType);
        var separator = '';
        if (TOI.STATE.activeType === 'letter') {
        } else if (TOI.STATE.activeType === 'word') {
            separator = ' ';
        } else if (TOI.STATE.activeType === 'paragraph') {
            separator = '\n';
        }
        TOI.STATE.sequence += str+separator;
    };
    var unsetTypeWrite = function(str) {
        var el = document.getElementsByClassName('marker')[0];
        el.className = 'frame-obj marker';

        var i = TOI.STATE.sequence.lastIndexOf(str);
        console.log('unset',str,'on',TOI.STATE.sequence,'at',i);
        if (i !== -1) {
    ...
    var updateFrame = function(xIn,yIn) {
        ...
        setActiveType(TOI.CONFIG.posMappings[lastIndex][index][1]);

        if (index !== lastIndex) {
            setContext(index, pointer);
        } else {
            if (document.getElementsByClassName('marker active').length === 0) {
